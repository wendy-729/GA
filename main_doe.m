...
% 遗传算法参数
% 遗传算法参数
para=[50,0.8,0.01;50,0.9,0.05;50,0.95,0.1;
    100,0.8,0.05;100,0.9,0.1;100,0.95,0.01;
    150,0.8,0.1;150,0.9,0.01;150,0.95,0.05];
[r,c]=size(para);
...
% 参数组合
for test_para=1:r
para_meter=para(test_para,:);
pop=para_meter(1);
p_cross=para_meter(2);
p_mutation=para_meter(3);
% 活动数量
for actN=[30]
...
fpathRoot=['C:\Users\ASUS\Desktop\DOE\J',actNumber,'\'];
% disp(actNumber)
dt=num2str(dtime);
outResults=[act,best_chrom(actNo+1),cputime,best_schedule,best_chrom(1:actNo),nr_schedules];
outFile=[fpathRoot,num2str(test_para),'\',num2str(end_schedules),'sch_',setName,'_dtime_end_',dt,'.txt'];
