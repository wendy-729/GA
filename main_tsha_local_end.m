...
%                    选择没有触发
    ...
    for i=1:pop 
        ...
        su_i=su;
        pred_i=pred;
        implement=child_implement(i,1:actNo); % 当前染色体
...
% fpathRoot=['C:\Users\ASUS\Desktop\回复审稿人实验\初始种群对比实验结果\J',actNumber,'\'];
% % 
% dt=num2str(dtime);
% outResults=[act,best_chrom(actNo+1),cputime,best_schedule,best_chrom,nr_schedules];
% outFile=[fpathRoot,groupdata,'\',num2str(end_schedules),'sch_',setName,'_dtime_',dt,'_tsha_end_para1','.txt'];
% % 
% outFile1=[fpathRoot,groupdata,'\',num2str(1000),'sch_',setName,'_dtime_',dt,'_tsha_end_para1','.txt'];
% outFile2=[fpathRoot,groupdata,'\',num2str(3000),'sch_',setName,'_dtime_',dt,'_tsha_end_para1','.txt'];
