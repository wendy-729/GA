...
%             disp(nr_schedules)
       ...
       %% 评价个体
        ...
        % 进行局部改进
        isflag=1;
        if count1==0 && nr_schedules+sum_tt>=1000
            isflag = 0;
        end
        if count2==0 && nr_schedules+sum_tt>=3000
            isflag = 0;
        end
...
% % fpathRoot=['E:\zlw\大修实验\实验结果\灵敏度分析\GA\J',actNumber,'\'];
% fpathRoot=['E:\zlw\大修实验\实验结果\GA\J',actNumber,'\'];
% % disp(actNumber)
% dt=num2str(dtime);
% outResults=[act,best_chrom(actNo+1),cputime,best_schedule,best_chrom,nr_schedules];
% outFile=[fpathRoot,groupdata,'\',num2str(end_schedules),'sch_',setName,'_dtime_correct2_',dt,'.txt'];
% outFile1=[fpathRoot,groupdata,'\',num2str(1000),'sch_',setName,'_dtime_correct2_',dt,'.txt'];
% outFile2=[fpathRoot,groupdata,'\',num2str(3000),'sch_',setName,'_dtime_correct2_',dt,'.txt'];
